...
async function get_status(req: Request, res: Response): Promise<Response> {
    try {
        ...
        if (athenasStatus.length === 0) {
            return res.status(404).json(createErrorResponse("[REACH-SDK - Athenas]: No Athenas status found.", 404));
        }
        ...
        console.error("[REACH-SDK - Athena]: Error retrieving Athena status:", error);
        return res.status(500).json(createErrorResponse("[REACH-SDK - Athenas]: An error occurred while retrieving Athena status.", 500));
    }
}
